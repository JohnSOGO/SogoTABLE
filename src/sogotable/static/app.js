...
async function exportReviewZip() {
  ...
  try {
    const entries = await reviewExportEntries();
...
async function reviewExportEntries() {
  const entries = [{
    name: "sogotable-review/REVIEW_EXPORT.md",
    bytes: encodeText(reviewExportReadme()),
  }];
...
function reviewExportReadme() {
  ...
  return [
    ...
    `App revision label: ${revisionText}`,
    "",
