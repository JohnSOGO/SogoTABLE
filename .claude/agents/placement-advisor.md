...
3. `workers/tests/architecture.test.js` — the actual enforced rules (read it so your
...
2. **Find the owner in the map.** Scan `module-ownership.md`'s owner rows and
   directory patterns. If exactly one owner's concern matches, that's the answer.
3. **No clean owner?** Decide between:
...
4. **God-file check (mandatory).** Look up the chosen target file's current line
...
5. **Layering / import constraints.** State any rule the implementer must honor for
...
6. **Sibling-path check.** If the concern has parallel implementations (hot-seat vs
