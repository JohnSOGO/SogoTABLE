...
test("lists ready games from the hosted game registry", async () => {
  ...
  assert.equal(listed.ok, true);
  assert.deepEqual(listed.games.map((game) => game.id), [CLASSIC_GAME_ID, TACTICAL_GAME_ID, BOXES_GAME_ID, BATTLESHIP_GAME_ID, QUORIDOR_GAME_ID, TEN_THOUSAND_GAME_ID, YAHTZEE_GAME_ID, MAZEWRIGHT_GAME_ID, ZOMBIE_DICE_GAME_ID, RTTA_GAME_ID]);
  assert.deepEqual(listed.games.map((game) => game.availability), ["ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready"]);
