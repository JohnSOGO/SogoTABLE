...
import {
  assertPlayerOwner, assertSogoSuperuser, assertSogoPasscode, isSogoSuperuser,
  configuredSogoSuperuserIds, generateOwnerToken, ownerTokenHash,
} from "./platform/auth.js";
import { reservedTestPlayerFromId } from "./test-players.js";
import {
  isHiddenPlayer, publicPlayers, publicPlayer, isHiddenTestRoom, publicBot, botDifficultyLevel, roomStatus,
} from "./projections.js";
import {
...
// Room outcome stats (completed-room recording, Elo, high scores, personal
// stats) — extracted owner module; the Worker only decides WHEN to call it.
import {
...
async function routeRequest(method, url, payload, data, options = {}) {
  ...
  if (method === "GET" && url.pathname === "/api/stats") {
    ...
    if (method === "POST" && url.pathname === "/api/bug-report") return appendBugReport(data, payload);
