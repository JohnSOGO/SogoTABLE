...
- Architecture guard tests (`workers/tests/architecture.test.js`) ratchet line-count ceilings (currently app 3350, worker 2050, css 2800), enforce registry single-source, and pin the review-export allowlist to tracked files. Lower a ceiling whenever code is extracted, and update these numbers here in the same change.
