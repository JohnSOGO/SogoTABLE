...
- Remaining:
  - Client adapters for Battleship (entangled: the stateful reveal-queue/timers/`battleshipViewMode` globals stay in the shell), 10,000 (render module exists; ~60 lines of sound/farkle glue + action handlers still in the shell), and Quoridor (entangled: 4 render-mutated globals + a pointer-timer wall-hold). These shrink `app.js` (now the larger god-file at 3893 lines); each needs its render-mutated globals lifted into module-private state.
