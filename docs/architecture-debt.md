...
- Architecture guard tests (`workers/tests/architecture.test.js`) ratchet line-count ceilings (currently app 2571, worker 1960, styles.css 1350, styles-games.css 1700), enforce registry single-source, pin the review-export allowlist to real files (git-tracked, or on-disk when `.git` is absent so an exported ZIP stays self-testable), and enforce import-closure (every relative import of an exported module is itself exported). Lower a ceiling whenever code is extracted, and update these numbers here in the same change.
