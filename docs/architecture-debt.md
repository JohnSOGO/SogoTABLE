...
- Remaining:
  ...
  - Capstone: convert `newGame`/`gameToDict`/`legalMoves`/`chooseBotMove`/`makeMove` from if/else chains into generic registry dispatch now that every game is a module.
