...
- `workers/sogotable-api.js` is now the request entry + routing + Durable Object classes + domain handlers; **every game's rules** and the **platform/persistence plumbing** live in modules; 4299 → ~1,800 lines, no inline game rules. HTTP/CORS (`workers/platform/http.js`), rate-limiting (`workers/platform/rate-limit.js`), and the D1 state row (`workers/persistence/state.js`) are extracted. Still mixing auth/lobby/rooms/invites/stats/bot-orchestration/notification fanout in the entry file — next target is per-domain handlers (`domains/`). Explicit per-route `{mutates, notifyRooms, notifyApp}` metadata (`ROUTE_SIDE_EFFECTS` + `sideEffectsFor`) has **replaced** the URL-string `readOnlyPost` heuristic; the dispatcher applies only the side effects a route declares.
- `src/sogotable/static/app.js` still owns most browser shell state, screen rendering, API orchestration, and most per-game interaction glue — but the registry, review-export, render-keys, storage, the boxes/quoridor/battleship client adapters, and the prompts/passcode controller, and the game-options/bug-report menu, and the game-stats panel/modal, and the win-overlay (`controllers/{prompts,game-options,game-stats,win-overlay}.js`), the player-edit appearance picker (`controllers/player-appearance.js`), and the Sogo superuser identity/passcode gate (`controllers/superuser.js`), the local hot-seat seat-persistence data layer (`controllers/local-seat.js`), and the room-snapshot→sound mapping (`controllers/room-sounds.js`), and the invite/opponent flows — host opponent picker (remote/bot/local) + the incoming-invite prompt (`controllers/invites.js`, wired via a ~18-entry ctx with setters for the shell-owned hostInviteStatus/activeGameRoom/selectedGameId) — are now modules; 4389 → ~2,500 lines. Still large for a shell; the remaining bulk is genuine shell orchestration (player CRUD/modal, room/lobby flows, the render fan) bound by shared mutable state (`playerModalMode`/`editingPlayerId`/`players`/`currentRoom`/`hostInviteStatus`); the cohesive leaf concerns are now extracted.
- Shell CSS is split across `styles.css` + `styles-roster.css` + `styles-room.css` + `styles-games.css`; per-game board CSS is injected by each game's render module (uncapped by design). No longer one broad stylesheet.
- Game timing/seating metadata is split across two sources: the runtime reads `games/registry.js` (`player_count`/`host_start`), while per-game `manifest.js` files carry richer `minPlayers`/`maxPlayers`/`timingMode`/`capabilities` that are not all runtime-consumed yet. Converge onto one source when a game needs a timing mode the registry can't express (e.g. a solo/real-time game). Documented as a known seam in `docs/adding-a-game.md`.
...
- Architecture guard tests (`workers/tests/architecture.test.js`) ratchet line-count ceilings (the live caps are the `CEILINGS` object in that test — read them there, not from a number copied here, which is how they rot), enforce registry single-source, pin the review-export allowlist to real files (git-tracked, or on-disk when `.git` is absent so an exported ZIP stays self-testable), and enforce import-closure (every relative import of an exported module is itself exported). Lower a ceiling whenever code is extracted — in `architecture.test.js`, the single source of truth.
...
- **Phase 2 is complete.** Every game's server rules and client UI are modules; the Worker dispatches through the `GAME_HANDLERS` registry. Adding a game is now: a rules module, a client module, a predicate, and one registry row — exercised again for game #7, **Roll Through the Ages** (the first live-round game; see `docs/game-rtta.md`), which landed with app.js *shrinking*, not growing.
...
1. ~~**Worker platform/persistence extraction**~~ — **DONE.** `platform/{http,cors,rate-limit}.js` and `persistence/state.js` (ensureSchema/loadState/saveState/withStateRetry) are out of `sogotable-api.js`; the Worker is now ~1,800 lines (well under the original 2,200 guard, since ratcheted far lower).
2. ~~**`routeRequest()` policy**~~ — **DONE.** Replaced the URL-string `readOnlyPost` heuristic with explicit per-route `{mutates, notifyRooms, notifyApp}` metadata (`ROUTE_SIDE_EFFECTS` + `sideEffectsFor`); the dispatcher applies only declared side effects. Tests pin a read-only POST (no write/notify) and a mutating POST (write + app-notify). Full co-location with handlers waits on the `domains/` handler extraction.
...
Smaller notes: review-export reflects GitHub `main`, not the deployed SHA — its README should state that caveat (and ideally pin the export to a commit SHA). D1 single-row debt stays acceptable for family scale; keep the exit criteria visible. CSS split has since landed (shell CSS across `styles.css`/`styles-roster.css`/`styles-room.css`/`styles-games.css`; per-game CSS injected).
