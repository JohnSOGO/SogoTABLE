...
- Client adapters: Boxes, 10,000 (renderer), and **Quoridor done & live-verified** (`games/quoridor/client.js`, board render + pawn move confirmed on the live site; its 4 UI globals are module-private). app.js 3893 → 3642.
- Remaining:
  - Battleship client adapter (the last one; entangled — the stateful reveal-queue/timers/`battleshipViewMode` globals stay in the shell, plus per-viewer hidden-info display; needs live verification of fleet placement, firing, reveals, and sinks).
  - Optional: 10,000's residual ~60 lines of sound/farkle glue (shell-integration code; low value).
  - Note: when adding a static game-module dir, check its tracked case (`git ls-files`) — a capital-cased lab dir (e.g. `Quoridor/`) collides with a lowercase import on Cloudflare's case-sensitive build and white-screens the app.
