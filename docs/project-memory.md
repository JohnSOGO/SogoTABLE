...
- Sogo superuser selection is passcode-gated. The browser must verify the passcode through the Worker before accepting `Sogo` or `MojoSOGO` as the selected player, and superuser room-close requests must include the Worker-configured `SOGOTABLE_SUPERUSER_PASSCODE`; if the secret is missing, superuser verification fails closed.
