...
- Reserved smoke-test player ids `codex-test-player-1` and `codex-test-player-2` are Worker-recognized hidden test seats. They can be used in API tests, but must remain filtered from the public roster, lobby presence, invite targets, and public stats so normal players cannot select them.
