...
- Public builds had a temporary localStorage player fallback before the hosted D1 brain was stable. When the shared player API is available, the browser should sync any locally saved fallback players into the shared roster so older PC/iPhone browsers do not appear to have separate player universes.
- Public room create/join actions must use the browser's device/home selected player and sync that player to the shared roster immediately before the action. Do not rely only on the last rendered player list; PC and iPhone browsers can be between polls or recovering old local fallback players.
