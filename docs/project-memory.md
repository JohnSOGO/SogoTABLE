...
- After the successful EventHub smoke test, app-level timed reads were reduced to fallback cadence: current-room summary every 15 seconds, selected-game lobby every 15 seconds, invites every 30 seconds, and active-room socket recovery every 15 seconds.
- Active room mutations for join, leave/close, move, and reset now route through `RoomDurableObject` before D1 persistence. Room creation and invite lifecycle are still Worker-owned for now. The next authority migration should target invites/creation only after this path is smoke-tested.
