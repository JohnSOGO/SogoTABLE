...
- `RoomDurableObject` is one live WebSocket fanout and room-mutation object per room. Active room-changing HTTP requests enter the room object first, then the object validates/persists through D1 and broadcasts the resulting room snapshot. This removes aggressive active-room polling during normal connected play while preserving HTTP as the reconnect/backfill path.
...
- After the Cloudflare quota fix, app-level timed reads are degraded fallback cadence: selected-game room summaries every 60 seconds, invites every 60 seconds, lobby presence every 60 seconds, and active-room socket recovery every 30 seconds. Presence heartbeat must not force a full room-list refresh every tick.
