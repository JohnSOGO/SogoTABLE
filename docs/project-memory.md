...
- Sogo is a superuser for room cleanup. While SogoTable has roster profiles but no full account/auth system, selecting a roster profile named `Sogo` or `MojoSOGO` exposes a confirmed close action on current-game cards and in the game header, and the Worker validates `POST /api/room/close` before closing arbitrary rooms.
- Sogo superuser selection is passcode-gated. The browser must verify the passcode through the Worker before accepting `Sogo` or `MojoSOGO` as the selected player unless Sogo is already unlocked as the current selected player in the tab session. Switching to any non-Sogo player clears that unlock, so returning to Sogo requires the passcode again. Superuser powers should only be visible after unlock, and superuser room-close requests must include the Worker-configured `SOGOTABLE_SUPERUSER_PASSCODE`; if the secret is missing, superuser verification fails closed.
