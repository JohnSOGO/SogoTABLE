...
- Active room mutations for join, bot join, invite response, leave/close, move, and reset now route through `RoomDurableObject` before D1 persistence. If the `ROOM_OBJECT` binding is unavailable, those production POST paths fail closed with `503 Room authority unavailable` instead of falling back to direct Worker mutation routing. Room creation and invite creation are still Worker-owned for now. The next authority migration should target room creation or deeper D1 state partitioning only after this path is smoke-tested.
