...
- The selected-game lobby shows the local selected player first, then other lobby-present players in sorted order. The game screen is the waiting/play surface. It shows Host and Opponent slots while waiting for the game to start, then hides that Players section once the game is active because turn ownership is shown separately. An empty opponent slot shows `Select Local Opponent` and `Invite Remote Opponent` for the host. Creating a game must open the actual tic-tac-toe game screen; while waiting for the second player, show the board disabled rather than hiding it. The game screen has a `Close` button that asks for Yes/No confirmation, deletes the current room/game so it is no longer listed or re-enterable, then returns all polling players to player/game selection. `Reset` also asks for Yes/No confirmation before clearing the board.
- Invited players receive an invite popup with `Yes` and `No`. Accepting joins the room and opens it; declining dismisses the invite.
