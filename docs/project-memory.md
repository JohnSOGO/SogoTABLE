...
- The first hosted brain is a Cloudflare Worker plus Workers KV namespace `sogotable-state`, configured by `wrangler.toml` and implemented in `workers/sogotable-api.js`. It mirrors the local Python API shape for players, lobby presence, rooms, invites, reset voting, and Super Tic Tac Toe moves so public browsers can see each other and play together. Durable Objects remain the better long-term architecture for stricter turn consistency, but KV is the pragmatic first deployable brain.
