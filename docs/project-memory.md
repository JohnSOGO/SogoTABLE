...
- The emergency hosted brain is a Cloudflare Worker configured by `wrangler.toml` and implemented in `workers/sogotable-api.js`. It mirrors the local Python API shape for players, lobby presence, rooms, invites, reset voting, and Super Tic Tac Toe moves so public browsers can see each other and play together. It currently stores state in Worker isolate memory because Workers KV hit the free daily write limit during normal lobby/player activity. This means public state can reset when the Worker isolate is evicted or redeployed. Durable Objects remain the correct next architecture for durable hosted multiplayer.
...
- When public phone/PWA behavior changes, bump the service worker `CACHE_NAME`. Old iPhone installs can keep stale `app.js` even after reinstall if the service worker cache name stays the same. Core shell files (`/`, `/index.html`, `/app.js`, `/styles.css`, `/revision.json`) should stay network-first.
