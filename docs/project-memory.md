...
- Player profiles now use lightweight browser-owned `owner_token` values instead of full accounts. `/api/players/create` returns a token once for new players, the Worker stores only a SHA-256 hash, and protected player/room/invite mutations require the token. Legacy unclaimed players can be claimed once through `/api/player/claim`. Public player objects expose `claimed` so the UI can show recovery affordances without exposing token hashes.
...
- Cipher and Overlord are the scored bots in the current difficulty ladder. They still use the hosted move pipeline, but their move choice scores each legal move for game wins, opponent game-win blocks, zone wins/blocks, threat shape, center/corner position, destination danger, and tactical pickup value. Keep this as move selection only; do not let bot logic mutate boards outside `makeMove`. The shared behavior contract now lives in `docs/bots/behavior.md`.
- In the bot invite UI, show `🧠` next to Cipher and Overlord and `🎲` next to random legal-move bots so the algorithm difference is visible without exposing ids.
...
- **Game-Locked category adopted (AREC 2026-06-27).** A new player-coupling axis *above* the within-game timing modes: instead of one shared board, each player plays their own independent game to completion and the shared truth is a leaderboard ("asynchronous multiplayer solitaire"). It is the supported path for solo-scoring games (Yahtzee, match-three) and resolves the earlier "solo/real-time doesn't fit the turn model" gap. The client owns its whole game and pushes only the global (committed scores + a tiny fixed set of significant plays that fan out to all players); in-progress turn state (dice/holds/current rolls) stays local. Score authority is intentionally softened for family scale (trusted client-posted scores; move/RNG-log re-validation is the upgrade path). N-player UI: own game on top, a tip/news strip for broadcast plays, an all-player score table at the bottom. A "series" wrapper (array of games → cumulative score → final winner) is a later phase, built only after single-game Game-Locked is solid. Reuses lobby/rooms/players/stats; it is a room mode, not a new lobby. Full doctrine in `docs/adding-a-game.md` (Sync modes). First implementation: Yahtzee, prototyped in `AI/Yahtzee`.
...
- Dependency installs are lockfile-based. Use `npm ci` for repeatable setup, `npm test` as the default Worker test alias, and GitHub Actions CI for `npm ci`, `npm run test:worker`, and `npm run bench:bots`.
- Public mutating API requests use Cloudflare Workers rate-limit bindings. `API_MUTATION_RATE_LIMITER` covers `POST`/`DELETE` at 180 requests per minute per client key, while `SUPERUSER_RATE_LIMITER` covers Sogo passcode verification at 20 attempts per minute per client key. Limited requests return HTTP `429` with `Retry-After: 60`.
- `RoomFactoryDurableObject` is the globally named room-creation authority. `POST /api/room/create` enters the factory first so code allocation and duplicate-code checks are serialized before D1 persistence. `RoomDurableObject` remains one live WebSocket fanout and room-mutation object per room. Active room-changing HTTP requests enter the room object first, then the object validates/persists through D1 and broadcasts the resulting room snapshot. This removes aggressive active-room refresh checking during normal connected play while preserving HTTP as the reconnect/backfill path.
...
- Active room mutations for join, bot join, invite create/respond, leave/close, move, and reset now route through `RoomDurableObject` before D1 persistence. Room creation routes through `RoomFactoryDurableObject`. If the required `ROOM_OBJECT` or `ROOM_FACTORY` binding is unavailable, those production POST paths fail closed with `503 Room authority unavailable` instead of falling back to direct Worker mutation routing. The next authority migration should target deeper D1 state partitioning only after this path is smoke-tested.
...
- Entering a selected-game lobby should happen quietly before the UI paints: register lobby presence and fetch lobby/current-games data first, then show the selected-game screen with settled lobby information to avoid visible placeholder-text toggles.
...
- Sogo is a superuser for room cleanup. While SogoTable has roster profiles but no full account/auth system, selecting a roster profile named `Sogo` or `MojoSOGO` exposes a confirmed close action on current-game cards and in the game header, but the Worker authorizes `POST /api/room/close` only when the requester id is listed in `SOGOTABLE_SUPERUSER_PLAYER_IDS`, the Sogo passcode is correct, and the request includes that player's owner token.
- Sogo superuser selection is passcode-gated. The browser must verify the passcode through the Worker before accepting `Sogo` or `MojoSOGO` as the selected player unless Sogo is already unlocked as the current selected player in the tab session. Switching to any non-Sogo player clears that unlock, so returning to Sogo requires the passcode again. Superuser powers should only be visible after unlock, but display-name detection is not server authority; superuser room-close requests must satisfy the configured player-id allowlist, owner token, and `SOGOTABLE_SUPERUSER_PASSCODE`. If the secret or allowlist is missing, superuser verification fails closed.
- Sogo superuser recovery includes `POST /api/player/unclaim`, which clears a player's stored owner-token hash so a browser that lost localStorage can reclaim the player. This endpoint is passcode-gated and intentionally does not require the lost owner token. The unlocked Sogo player sees `Unlock` on claimed player rows.
- Sogo superuser invite cleanup includes `POST /api/invite/cancel`, which deletes pending invites for a stuck target and can optionally narrow by host. It is passcode-gated and broadcasts app snapshots, but currently has no dedicated browser control.
- When Sogo is the unlocked selected player, the intro screen may show an `Export .ZIP` action below `Start Playing`. This is a source-review convenience export, not a full backup: keep it allowlist-based and limited to public source/docs safe for external review. It must exclude secrets, `.wrangler`, runtime/D1 state, dependency folders, generated caches, and ignored `AI/` intake files.
...
- Battleship is a hidden-information game. Worker HTTP and room WebSocket responses are projected per `player_id`: the viewer receives their own ships and public shot results, but opponent ship coordinates stay hidden until the room is complete. The exception is sunk enemy ships: once the viewer fully sinks an opponent ship, the Worker reveals that ship so the browser can mark it red and name it in the sunk-ship popup. The browser must render hit/miss from authoritative room snapshots instead of previewing attacks from opponent ship data.
...
- The global recruitable AI ladder is Sprout (Novice), Buddy (Casual), Cipher (Strategist), and Overlord (Master). Quoridor maps those four bot ids onto its local four-difficulty model in `src/sogotable/static/games/Quoridor/quoridor_ai_rules_four_difficulties.md`.
...
- Uncommitted 10,000 dice selections persist across full room snapshot re-renders caused by other parallel seats acting. Store that client-only highlight by room, mark, and roll count, and clear it when the local player commits a selection, banks, or declares a farkle.
...
- 10,000 publishes a canonical `finish_state` per seat. The standings table header is `Status`, and the UI maps the state to hourglass, green check, red X, or both after a farkle ACK. The farkle tray also exposes an OK button so the seat cannot soft-lock if the popup is missed.
- The top banner for 10,000 shows the round number, and the tray below the actions stays quiet so the play surface is not cluttered with status copy.
- After every seat has banked or farkled and acknowledged, the round stays on the current board until the next roll starts the following round.
...
- On phones, 10,000 keeps six dice 3-across and action buttons 4-across. Shrink controls to preserve that scan pattern instead of reflowing dice/actions into fewer columns.
- 10,000 action buttons default to emoji labels, but Game Options can persist a per-device `sogotable.actionLabels = words` preference for short word labels. Keep words compact enough for a single 4-across row on small phones.
- 10,000 sound effects are tied to the local device's own room seat. Parallel snapshots for other seats must update state without playing this device's roll, score, bank, or farkle sounds.
