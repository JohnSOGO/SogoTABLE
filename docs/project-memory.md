...
- The game screen has a `Close` button that asks for Yes/No confirmation, deletes the current room/game so it is no longer listed or re-enterable, then returns all polling players to player/game selection. `Reset` also asks for Yes/No confirmation before clearing the board; after a completed game the same control is labeled `Play Again` and starts a fresh board with the same seated players.
- Invited players receive an invite popup with `Yes` and `No`. Accepting joins the room and opens it; declining dismisses the invite. Hosts should see invite lifecycle feedback while waiting: sent, accepted, declined, or expired.
